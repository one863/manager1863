...
export function getNarrative(category: string, subCategory: string, params: NarrativeParams = {}): { title?: string, content: string } {
  // i18next permet de récupérer des tableaux ou des objets avec returnObjects: true
  const narratives = i18next.t(`narratives.${category}.${subCategory}`, { returnObjects: true, defaultValue: [] }) as any;
  ...
  if (Array.isArray(narratives)) {
    ...
    if (typeof selected === 'string') {
      return { content: i18next.t(`narratives.${category}.${subCategory}.${narratives.indexOf(selected)}`, params) };
    }
  } 
  ...
  if (typeof narratives === 'object') {
    ...
    if (narratives.title && narratives.content) {
      ...
      const contentIndex = Math.floor(Math.random() * narratives.content.length);
      
      return {
        title: i18next.t(`narratives.${category}.${subCategory}.title.${titleIndex}`, params),
        content: i18next.t(`narratives.${category}.${subCategory}.content.${contentIndex}`, params)
      };
    }
    
    // Si c'est un tableau d'objets (ex: sponsors ou weekly)
    const keys = Object.keys(narratives).filter(k => !isNaN(Number(k)));
    if (keys.length > 0) {
      const randomKey = getRandomElement(keys);
      const item = narratives[randomKey];
      return {
        title: i18next.t(`narratives.${category}.${subCategory}.${randomKey}.title`, params),
        content: i18next.t(`narratives.${category}.${subCategory}.${randomKey}.content`, params)
      };
    }
  }
