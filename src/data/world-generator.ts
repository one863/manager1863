	...
	const tutorials = [
		{
			...
			content: `Monsieur le Manager, au nom du Conseil d'Administration, je vous souhaite la bienvenue. 
...
Nous attendons de vous de la rigueur et des résultats.`,
		...
		{
			day: 2,
			title: "ÉTAT DE L'EFFECTIF & MERCATO",
			content: `La revue d'effectif est terminée. Nos scouts ont listé les forces en présence dans l'onglet **Effectif**. 
...
Sachez que le marché des transferts est ouvert. Si vous estimez que certains joueurs n'ont pas le niveau requis pour porter nos couleurs, vous pouvez chercher des renforts dans le **Marché**. Gardez un œil sur la condition physique des joueurs avant chaque décision.`,
		...
		{
			day: 3,
			title: "ENCADREMENT ET DÉVELOPPEMENT",
			content: `Un bon manager sait s'entourer. Vérifiez les compétences de votre **Staff** actuel. Des préparateurs compétents accéléreront la progression de vos joueurs. 
...
Le recrutement ne s'arrête pas aux joueurs : un scout de talent ou un coach chevronné peut faire la différence entre une promotion et une relégation.`,
		...
		{
			day: 4,
			title: "ACTUALITÉS ET RÉPUTATION",
			content: `La presse locale commence à parler de votre nomination. Les **Actualités** sont votre lien avec le monde extérieur. 
...
Soyez attentif aux dépêches concernant vos adversaires ou les opportunités financières. Votre réputation influencera directement le type de sponsors qui frapperont à notre porte.`,
		},
		{
			...
			content: `Notre place est dans l'élite, mais le chemin sera long. Le **Classement** de notre ligue est désormais disponible. 
...
Analysez nos futurs adversaires et leurs résultats. Chaque point compte pour atteindre l'objectif de saison que nous vous avons fixé. Ne nous décevez pas.`,
		...
		{
			day: 6,
			title: "PRÉPARATION AU MATCH",
			content: `Le match approche. Le système de **Match Live** vous permettra de diriger l'équipe en temps réel. 
...
N'oubliez pas d'ajuster votre **Tactique** et de désigner vos 11 titulaires. Un joueur fatigué (énergie basse) sur le terrain est un risque que nous ne pouvons pas nous permettre.`,
		},
		{
			...
			content: `C'est le jour du bilan. Chaque semaine (jours 7, 14, 21...), nous faisons les comptes dans l'onglet **Trésorerie**. 
...
Les revenus des sponsors et de la billetterie seront versés aujourd'hui, après déduction des salaires. Assurez-vous que le solde reste positif pour garantir la pérennité du club.`,
	...
	// On ajoute les messages pour chaque jour
	const newsItems: NewsArticle[] = tutorials.map(
		(tuto) =>
			({
				saveId,
				day: tuto.day,
				date: new Date(currentYear, 8, tuto.day),
				title: tuto.title,
...
export const WorldGenerator = {
	async generateWorld(
		...
		for (let level = 1; level <= DIVISIONS; level++) {
			...
			const teamsCount =
				level === DIVISIONS ? TEAMS_PER_DIV - 1 : TEAMS_PER_DIV; // Une place pour le joueur en Div 5
			...
			if (level === DIVISIONS) {
				userTeamId = (await db.teams.add({
					...
					goalsAgainst: 0,
					goalDifference: 0,
					budget: 1000, // Augmenté de 250 à 1000
					reputation: 20,
					fanCount: 150,
					confidence: 50,
