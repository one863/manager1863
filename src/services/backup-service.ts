...
export const BackupService = {
	async performAutoBackup(saveId: number) {
		try {
			...
			console.log(
				`[Backup] Auto-sauvegarde locale effectuée pour le slot ${saveId}`,
			);

			// 2. Sauvegarde Cloud (si connecté)
			if (CloudService.getCurrentUser()) {
				const gameState = await db.gameState.get(saveId);
				const team = gameState?.userTeamId
					? await db.teams.get(gameState.userTeamId)
					: null;
				...
				if (gameState) {
					...
					console.log(
						`[Backup] Synchronisation Cloud effectuée pour le slot ${saveId}`,
					);
				}
			}
		} catch (e) {
