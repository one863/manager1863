...
export const CloudService = {
  ...
  // Sauvegarde (Upload)
  async uploadSave(saveId: number, saveDataJSON: string, metadata: { clubName: string, season: number, day: number }) {
    if (!auth || !db) return false;
    
    const user = auth.currentUser;
    if (!user) throw new Error("Vous devez être connecté pour sauvegarder dans le Cloud.");
