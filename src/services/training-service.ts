...
export const TrainingService = {
   ...
   */
  async trainSquad(saveId: number, teamId: number, focus: 'PHYSICAL' | 'TECHNICAL' | 'TACTICAL') {
    ...
    for (const player of players) {
      ...
      if (probability(chance)) {
        ...
        const avgSkill = Math.round(
          (stats.speed + stats.strength + stats.stamina + stats.shooting + stats.passing + stats.dribbling + stats.defense) / 7
        );
