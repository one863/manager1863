...
export const useGameStore = create<GameState>((set, get) => ({
	...
	loadGame: async (slotId) => {
		...
		try {
			...
			// FIXED: Look for gameState by saveId index, not primary key
	...
	finalizeLiveMatch: async () => {
		...
		set({ day: finalDay, season: finalSeason, currentDate: nextDate, isProcessing: false, lastUpdate: Date.now() });
		await get().refreshUnreadNewsCount();
	},
