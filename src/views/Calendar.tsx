...
export default function Calendar() {
  ...
  matches.forEach(match => {
    const monthKey = match.date.toLocaleDateString(undefined, { month: 'long', year: 'numeric' });
  ...
  return (
    <div className="pb-20 space-y-6">
      <div className="flex justify-between items-center mb-4 px-2">
        <h2 className="text-xl font-serif font-bold text-ink">{t('calendar.title')}</h2>
      </div>
      ...
      {matches.length === 0 ? (
        <div className="p-8 text-center bg-white rounded border border-gray-200 space-y-2">
          ...
          <p className="text-xs text-red-500">
            (Si vous venez de créer une partie, assurez-vous d'avoir utilisé "Nouvelle Carrière" après la dernière mise à jour)
          </p>
        </div>
      ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 flex items-center justify-between px-2">
                      <span className={`flex-1 text-right text-sm ${match.homeTeamId === userTeamId ? 'font-bold text-accent' : 'text-ink'}`}>
                        {homeTeam?.name || '???'}
                      </span>
                      
                      <div className={`px-3 font-mono font-bold text-sm rounded mx-2 min-w-[3rem] text-center py-1 border 
                      ...
                      <span className={`flex-1 text-left text-sm ${match.awayTeamId === userTeamId ? 'font-bold text-accent' : 'text-ink'}`}>
                        {awayTeam?.name || '???'}
                      </span>
                    </div>
                  </div>
