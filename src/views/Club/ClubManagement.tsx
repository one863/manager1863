...
export default function ClubManagement() {
  ...
  const [showProjectConfirm, setShowProjectConfirm] = useState<'UPGRADE' | 'NEW_STADIUM' | null>(null);
  ...
  const handleStartProject = async (type: 'UPGRADE' | 'NEW_STADIUM') => {
    if (!userTeamId) return;
    const result = await ClubService.startStadiumProject(userTeamId, currentDay, type);
    if (result.success) {
    ...
    <div className="space-y-6 pb-24 animate-fade-in relative px-1">
      ...
      <Card title="Conseil d'Administration">
        <div className="space-y-6">
          <ProgressBar label="Soutien du Board" value={team.confidence || 0} icon={Heart} color={(team.confidence || 0) > 50 ? 'bg-green-600' : 'bg-red-500'} />
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-3 p-3 bg-paper-dark/40 rounded-2xl border border-white/50 shadow-inner">
              <div className="p-2 bg-white rounded-lg text-accent shadow-sm"><Users size={16} /></div>
              <div>
                <span className="text-[9px] uppercase text-ink-light font-black block leading-none mb-1">Joueurs</span>
                <span className="font-bold text-sm text-ink">{squadSize}</span>
              </div>
            </div>
            <div className="flex items-center gap-3 p-3 bg-paper-dark/40 rounded-2xl border border-white/50 shadow-inner">
              <div className="p-2 bg-white rounded-lg text-yellow-600 shadow-sm"><Award size={16} /></div>
              <div>
                <span className="text-[9px] uppercase text-ink-light font-black block leading-none mb-1">Palmarès</span>
          ...
          <div className="flex items-start gap-4 p-2">
            ...
            <div className="flex-1 pt-1">
              ...
              <div className="flex items-center gap-3 text-xs text-ink-light mt-1.5 font-bold uppercase tracking-wider">
                <span className="bg-paper-dark px-2.5 py-1 rounded-full border border-gray-100">Niv. {stadiumLevel}</span>
                <span className="flex items-center gap-1"><Users size={12} className="opacity-50" /> {team.stadiumCapacity} PLACES</span>
              </div>
            </div>
          </div>
            ...
            <div className="bg-accent/5 border-2 border-accent/20 rounded-2xl p-5 shadow-sm relative overflow-hidden">
               <div className="flex justify-between items-center mb-3">
                  <div className="flex items-center gap-2">
                     ...
                     <span className="text-xs font-black text-ink uppercase tracking-widest">{team.stadiumProject?.type === 'NEW_STADIUM' ? 'Construction Nouveau Stade' : 'Agrandissement en cours'}</span>
                  </div>
               ...
               <div className="h-2 bg-paper-dark rounded-full overflow-hidden shadow-inner">
                  <div className="h-full bg-accent animate-pulse" style={{ width: `${((totalDays - upgradeDaysLeft) / totalDays) * 100}%` }}></div>
               </div>
            </div>
          ) : (
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="p-2.5 bg-white rounded-xl group-hover:bg-accent group-hover:text-white transition-colors shadow-sm"><Construction size={20} /></div>
                  <div className="text-left">
                    <span className="block text-xs font-black text-ink uppercase tracking-widest">Agrandir les tribunes</span>
                    <span className="text-[10px] text-ink-light italic opacity-60">+1000 places • 30 jours</span>
                  </div>
                </div>
              ...
              {stadiumLevel >= 2 && (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="p-2.5 bg-accent/10 text-accent rounded-xl group-hover:bg-accent group-hover:text-white transition-colors shadow-sm"><Hammer size={20} /></div>
                    <div className="text-left">
                      <span className="block text-xs font-black text-ink uppercase tracking-widest">Nouveau Stade Moderne</span>
                      <span className="text-[10px] text-ink-light italic opacity-60">10 000 places • 60 jours</span>
                    </div>
                  </div>
      ...
      <Card title="Vision du Club">
        <div className="space-y-4 py-1">
          <div className="flex items-center gap-4 p-3 rounded-2xl bg-paper-dark/30 border border-white/50 group">
            <div className={`w-3 h-3 rounded-full shadow-sm ${team.reputation >= 50 ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`}></div>
            <div className="flex-1">
              <p className="text-[10px] text-ink-light font-black uppercase tracking-widest leading-none mb-1">Image de Marque</p>
              <p className="text-xs text-ink font-bold">Réputation stable (50+)</p>
            </div>
          </div>
          <div className="flex items-center gap-4 p-3 rounded-2xl bg-paper-dark/30 border border-white/50">
            ...
            <div className="flex-1">
              <p className="text-[10px] text-ink-light font-black uppercase tracking-widest leading-none mb-1">Ambition Sportive</p>
              <p className="text-xs text-ink font-bold uppercase tracking-tight">
                {team.seasonGoal ? t(`season_goals.${team.seasonGoal}`) : t('season_goals.MID_TABLE')}
              </p>
            </div>
          </div>
        </div>
