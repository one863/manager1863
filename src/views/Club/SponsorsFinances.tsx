...
export default function SponsorsFinances() {
  ...
  const loadData = async () => {
    ...
    if (teamData) {
      const sponsorOffers = await ClubService.getSponsorOffers(
        teamData.reputation,
        currentDate,
      );
  ...
  const handleSignSponsor = async (offer: any) => {
    ...
    alert(
      `Contrat signé avec ${offer.name} jusqu'en ${offer.expiryDate.toLocaleDateString()} !`,
    );
    loadData();
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-24">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <div className="flex items-center gap-1 text-xs text-ink-light italic">
                <Calendar size={12} />
                Expire le{' '}
                {new Date(team.sponsorExpiryDate!).toLocaleDateString()}
              </div>
            </div>
            <div className="text-right">
              <CreditAmount amount={team.sponsorIncome || 0} size="lg" />
              <span className="text-[10px] block uppercase text-ink-light">
                par match
              </span>
            </div>
          </div>
      ...
      <div className="space-y-3">
        ...
        <p className="text-[10px] text-ink-light px-2 italic">
          Note : Signer un nouveau contrat remplacera l'actuel.
        </p>
      ...
      <Card title="Prévisions de Revenus">
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <CreditAmount
              amount={Math.round(team.fanCount * 0.1)}
              size="sm"
              color="text-ink"
            />
          </div>
          ...
          <div className="border-t border-gray-100 pt-2 flex justify-between font-bold">
            ...
            <CreditAmount
              amount={
                Math.round(team.fanCount * 0.1) + (team.sponsorIncome || 0)
              }
              size="md"
            />
          </div>
