...
export default function LeagueTable() {
  ...
  useEffect(() => {
    const loadTable = async () => {
      ...
      // Charger les équipes
      const teams = await db.teams.where('saveId').equals(currentSaveId).toArray();
  ...
  return (
    <div className="pb-20">
      <div className="flex justify-between items-center mb-4 px-2">
        <h2 className="text-xl font-serif font-bold text-ink">{t('league_table.title')}</h2>
      </div>
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-paper-dark text-xs uppercase font-bold text-ink-light border-b border-gray-300">
            <tr>
              ...
              <th className="px-2 py-2 text-center w-8">{t('league_table.p')}</th>
              <th className="px-2 py-2 text-center w-8 font-bold text-ink">{t('league_table.pts')}</th>
            </tr>
          </thead>
