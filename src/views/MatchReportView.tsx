...
import { ArrowLeft, Download, Copy, Check } from 'lucide-preact';
...
export default function MatchReportView({ matchId, onBack }: MatchReportViewProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      matchData.details.events.forEach(e => {
        if (e.type === 'GOAL') {
          if (e.teamId === matchData.homeTeamId) hScorers.push({ name: e.scorerName || '???', minute: e.minute });
  ...
  if (isLoading || !match || !homeTeam || !awayTeam) {
    return <div className="p-8 text-center animate-pulse">{t('game.loading')}</div>;
  }
    ...
    <div className="flex flex-col h-full bg-white font-sans relative animate-fade-in">
      ...
      <div className="absolute top-0 left-0 w-full p-3 z-50 flex justify-between pointer-events-none">
        <div className="pointer-events-auto flex gap-3">
          <button onClick={onBack} className="text-gray-400 hover:text-black transition-colors" title="Retour">
            <ArrowLeft size={18} />
          </button>
        </div>
        <div className="pointer-events-auto flex gap-3">
          <button onClick={downloadLogs} className="text-gray-300 hover:text-gray-900 transition-colors">
            <Download size={16} />
          </button>
          <button onClick={copyLogs} className="text-gray-300 hover:text-gray-900 transition-colors">
