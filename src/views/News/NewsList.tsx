...
import { Newspaper, Bell, Info, ArrowLeft, CheckCircle, Target, Building2 } from 'lucide-preact';
...
export default function NewsView() {
  ...
  useEffect(() => {
    const loadNews = async () => {
      ...
      // On récupère toutes les news triées par date décroissante
  ...
  if (selectedArticle) {
    return (
      <div className="space-y-4 animate-fade-in pb-24">
        <button onClick={() => setSelectedArticle(null)} className="flex items-center gap-2 text-ink-light hover:text-accent transition-colors p-2">
          <ArrowLeft size={18} /> <span>Retour aux dépêches</span>
        </button>

        <Card className="min-h-[50vh] flex flex-col shadow-xl border-accent/20">
          ...
          <div className="flex-1 text-ink leading-relaxed font-serif text-lg whitespace-pre-wrap italic">
  ...
  return (
    <div className="space-y-4 animate-fade-in pb-24">
      <div className="flex items-center justify-between px-2">
        ...
        <span className="text-xs text-ink-light italic">Dépêches du jour</span>
      </div>
        ...
        <div className="space-y-3">
          {articles.map((article) => (
            <div
              ...
              className={`
                cursor-pointer transition-all hover:translate-x-1
